...
FastEnemy::FastEnemy(sf::Texture* texture, sf::Vector2u imageCount
                     )
    : Enemy(texture, imageCount, 2.0f, 100.0f, 5.0f) {
  body.setSize(sf::Vector2f(50.0f, 50.0f));  // the size of the enemy
  body.setTexture(texture);                  // the texture of the enemy
}

// inherited function from enemy, which will have its movement changed as it is
// faster
